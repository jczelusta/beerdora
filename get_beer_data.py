...
_DIR = "/Users/admin/Documents/Beerdora/beeradvocate/beeradvocate.com/user/"
...
def process_rows(rows, result):
    ...
    for row in rows:
        ...
        result["ba_beer_id"].append(cols[2].split('<a href="')[1].split("/?ba=" + username)[0].split("/")[-1])
        result["brewery"].append(cols[2].split('</a>')[1].split(">")[-1])
        result["ba_brewery_id"].append(cols[2].split('<a href="')[2].split('">')[0].split("/")[-1])
        result["style"].append(cols[2].split("</a>")[-2].split('">')[-1])
        result["ba_style_id"].append(cols[2].split('<a href="/beer/style/')[1].split('">')[0])
        abv = cols[3].split(">")[-1]
...
def main():
    ...
    for filename in os.listdir(_DIR):
        if filename[:5] == _PREFIX and len(filename) > 5 and "&view" not in filename:
            try:
